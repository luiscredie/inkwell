#!/usr/bin/env python3
"""LigaLorcana full-catalog marketplace price collector (daily v5).
...
Modes:
  ...
  * Stop quickly and safely when the website returns repeated HTTP 401/403.
...
class AccessBlockedError(RuntimeError):
    """Raised when the website refuses automated access (HTTP 401/403)."""
...
def should_refresh(
    ...
    retry_errors_today: bool = False,
) -> bool:
    if refresh_all or result is None:
        return True
    attempted = parse_iso(result.last_attempt_at or result.checked_at)
    if attempted is not None and attempted.tzinfo is None:
        attempted = attempted.replace(tzinfo=timezone.utc)
    if refresh_today and attempted is not None:
        attempted_today = (
            attempted.astimezone(LOCAL_TIMEZONE).date()
            == datetime.now(LOCAL_TIMEZONE).date()
        )
        attempt_status = result.last_attempt_status or result.status
        if attempted_today and not (
            retry_errors_today and attempt_status == "error"
        ):
            return False
    ...
    for attempt in range(retries + 1):
        ...
        try:
            ...
            if status_code in {401, 403}:
                raise AccessBlockedError(f"Website returned HTTP {status_code}")
            try:
            ...
            # Retrying the same denied request only wastes time and can extend
            # the block. The collection loop owns the circuit breaker.
            last_error = exc
            break
        except Exception as exc:
            last_error = exc
...
def merge_attempt(
    previous: CardResult | None,
    attempt: CardResult,
) -> CardResult:
    """Record an attempt without replacing a previously valid price band."""
    attempt.last_attempt_at = attempt.checked_at
    attempt.last_attempt_status = attempt.status
    attempt.last_attempt_error = attempt.error
    if attempt.status != "error" or previous is None or previous.status == "error":
        return attempt
    previous.last_attempt_at = attempt.checked_at
    previous.last_attempt_status = "error"
    previous.last_attempt_error = attempt.error
    return previous


def is_access_block_result(result: CardResult) -> bool:
    return bool(
        result.status == "error"
        and result.error
        and re.search(r"HTTP\s+(?:401|403)\b", result.error, re.I)
    )


def automatic_finalize_allowed(payload: dict[str, Any], errors_this_run: int) -> bool:
    return (
        errors_this_run == 0
        and int(payload.get("summary", {}).get("errors") or 0) == 0
    )
...
def build_resume_status(output_path: Path, cards_path: Path) -> dict[str, Any]:
    ...
    for item in raw_cards:
        ...
        attempt_status = str(item.get("last_attempt_status") or status)
        attempt_status_counts[attempt_status] = attempt_status_counts.get(attempt_status, 0) + 1
        checked = parse_iso(item.get("last_attempt_at") or item.get("checked_at"))
    ...
    return {
        ...
        "last_attempt_status_counts": attempt_status_counts,
...
async def collect_prices(
    ...
    retry_errors_today: bool, access_block_threshold: int,
    ...
    fetched = errors = retries_count = consecutive_access_blocks = 0
    page = await context.new_page()
    ...
    for index, card in enumerate(pending, start=1):
        ...
        previous = existing.get(result.id)
        stored = merge_attempt(previous, result)
        existing[result.id] = stored
        fetched += 1
        if result.status == "error":
            errors += 1
        ...
        if is_access_block_result(result):
            consecutive_access_blocks += 1
        else:
            consecutive_access_blocks = 0
        if consecutive_access_blocks >= max(1, access_block_threshold):
            await save_raw_checkpoint()
            await page.close()
            logging.error(
                "Access circuit breaker opened after %s consecutive HTTP "
                "401/403 responses. Cached good prices were preserved and "
                "derived artifacts were NOT republished. Wait before "
                "running --resume-today again; do not loop denied requests.",
                consecutive_access_blocks,
            )
            return 3
    await page.close()
    payload = await save_raw_checkpoint()
    stored_errors = int(payload.get("summary", {}).get("errors") or 0)
    if not automatic_finalize_allowed(payload, errors):
        logging.error(
            "Collection checkpoint saved, but derived artifacts were NOT "
            "republished because this run had %s error(s) and the durable "
            "cache contains %s error record(s). Resume later. Use "
            "--finalize-cache only if you intentionally accept the current "
            "mixed-age cache.",
            errors,
            stored_errors,
        )
...
async def run(args: argparse.Namespace) -> int:
    ...
    async with async_playwright() as pw:
        ...
        result = await collect_prices(
            ...
            args.retry_errors_today, max(1, args.access_block_threshold),
            only_ids or None,
        )
...
def main() -> int:
    ...
    parser.add_argument("--delay-min", type=float, default=8.0, help="Minimum delay between requests")
    parser.add_argument("--delay-max", type=float, default=30.0, help="Maximum adaptive delay")
    parser.add_argument("--rate-limit-cooldown", type=float, default=900.0, help="Initial cooldown after HTTP 429")
    parser.add_argument("--timeout", type=int, default=30000)
    parser.add_argument("--retries", type=int, default=2, help="Retries for transient errors; HTTP 401/403 are never retried")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "With --resume-today, retry records whose last attempt today "
            "failed. Off by default to prevent immediate 403 retry loops."
        ),
    )
    parser.add_argument(
        "--access-block-threshold",
        type=int,
        default=3,
        help=(
            "Stop safely after this many consecutive HTTP 401/403 responses "
            "(default: 3); cached good prices remain intact"
        ),
    )
    parser.add_argument(
        "--only-id",
    ...
    if args.retry_errors_today and not args.refresh_today:
        parser.error("--retry-errors-today requires --resume-today")
    if args.show_paths:
        print(json.dumps({
